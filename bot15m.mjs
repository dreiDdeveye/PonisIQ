...
async function publishLivePrediction({ direction, ptb, btcPrice, confidence, confPct, bullScore, bearScore, signalsText }) {
  ...
  const row = {
    id: LIVE_PRED_ID, window_start: windowStart, direction: direction || 'pending',
...
async function discordPrediction(direction, confLabel, confPct, bullScore, bearScore, signals) {
  ...
  await sendDiscord({
    username: 'Vanguard 15M Bot', avatar_url: BOT_AVATAR_URL,
    embeds: [{
...
async function tick() {
  ...
  // Place vote at 50% remaining (faster prediction)
  const voteMark = Math.floor(WINDOW_SECS * 0.50);
