...
(function () {
  ...
  const stackOffsets = ['-18px', '10px', '-8px', '18px'];

  arsenalUnits.forEach((unit, i) => {
    const card = document.createElement('div');
    card.className = 'arsenal-card reveal';
    card.dataset.delay = String((i % 3) + 1);
    card.dataset.rarity = unit.rarity.toLowerCase();
    card.style.setProperty('--stack-offset', stackOffsets[i % stackOffsets.length]);
    ...
    card.innerHTML = `
      ...
      <div class="arsenal-card-grid"></div>
      <div class="arsenal-card-scan"></div>
      <div class="arsenal-card-info">
        <div class="arsenal-card-topline">
          <span class="arsenal-card-rank">${unit.rank}</span>
          <span class="arsenal-card-series">ABSTRACT CHAIN</span>
        </div>
