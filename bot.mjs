...
const BOT_AVATAR_URL = process.env.BOT_AVATAR_URL || 'https://pbs.twimg.com/profile_images/1956229874282373120/qirXy0XJ_400x400.jpg';
...
async function publishLivePrediction({ direction, ptb, btcPrice, confidence, confPct, bullScore, bearScore, signalsText }) {
  const windowStart = Math.floor(Math.floor(Date.now() / 1000) / WINDOW_SECS) * WINDOW_SECS;
  const row = {
    id: 1, window_start: windowStart, direction: direction || 'pending', ptb: ptb || null, btc_price: btcPrice || null,
...
async function discordPrediction(direction, confLabel, confPct, bullScore, bearScore, signals) {
  ...
  const wr = total > 0 ? ((state.wins / total) * 100).toFixed(1) : '0';
  await sendDiscord({
    username: 'Vanguard Bot', avatar_url: BOT_AVATAR_URL,
    embeds: [{
...
async function discordResult(correct, predictedOver, ptb, endPrice) {
  ...
  const wr = total > 0 ? ((state.wins / total) * 100).toFixed(1) : '0';
  await sendDiscord({
    username: 'Vanguard 5Min Bot', avatar_url: BOT_AVATAR_URL,
    embeds: [{
...
async function tick() {
  ...
  // Place vote at 50% remaining (faster prediction) — 150 seconds for 5m window
  if (!state.predictionMade && timeLeft <= 150 && timeLeft > 0 && state.priceToBeat) {
